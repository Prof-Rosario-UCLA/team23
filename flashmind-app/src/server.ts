...
app.post('/api/generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      res.status(502).json({ error: 'OpenAI did not return JSON', raw: rawContent });
      return;
    }
    ...
    if (
      ...
      res.status(502).json({ error: 'unexpected OpenAI response', raw: rawContent });
      return;
    }

    // Cast is safe after above checks
    const cards = (parsed as { cards: { front: string; back: string }[] }).cards;

    // return in the required envelope
    res.json({ cards });
  } catch (err) {
